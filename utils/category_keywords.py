"""
Category Keywords for Special Days Classification

Centralized keyword lists for categorizing special days/holidays into
Global Health, Tech, and Culture categories. Used by both UN Observances
and Calendarific clients.
"""

# Health-related keywords (merged from UN and Calendarific sources)
HEALTH_KEYWORDS = [
    # Diseases and conditions
    "health",
    "disease",
    "cancer",
    "diabetes",
    "tuberculosis",
    "tb",
    "malaria",
    "hiv",
    "aids",
    "hepatitis",
    "epilepsy",
    "alzheimer",
    "parkinson",
    "obesity",
    "stroke",
    "chagas",
    "leprosy",
    "polio",
    "measles",
    "cholera",
    "ebola",
    "yellow fever",
    "rabies",
    "rare disease",
    # Mental health
    "mental",
    "suicide",
    "autism",
    # Medical terms
    "medical",
    "medicine",
    "patient",
    "nurse",
    "nursing",
    "hospital",
    "drug",
    "tobacco",
    "antimicrobial",
    "antibiotic",
    # Vaccination and immunization
    "immunization",
    "vaccine",
    "vaccination",
    # Body parts and organs
    "heart",
    "blood",
    "donor",
    "organ",
    "transplant",
    "kidney",
    "liver",
    "lung",
    "brain",
    # Public health
    "epidemic",
    "pandemic",
    "hygiene",
    "sanitation",
    "safety",
    "drowning",
    "snakebite",
    # Disabilities and conditions
    "disability",
    "down syndrome",
    "albinism",
    "hearing",
    "sight",
    "blindness",
    # Maternal and child health
    "nutrition",
    "breastfeeding",
    "maternal",
    "child health",
    # Organizations
    "world health",
    "who ",
    "world health organization",
]

# Technology and science keywords (merged from UN and Calendarific sources)
TECH_KEYWORDS = [
    # Computing and internet
    "internet",
    "computer",
    "digital",
    "technology",
    "programming",
    "software",
    "hardware",
    "data",
    "web",
    "online",
    "cyber",
    # AI and robotics
    "artificial intelligence",
    "ai ",
    "robot",
    # Communications
    "telecommunication",
    "information",
    "radio",
    "television",
    "satellite",
    # Sciences
    "science",
    "scientific",
    "engineering",
    "mathematics",
    "math",
    "logic",
    "physics",
    "chemistry",
    "biology",
    "genetics",
    # Space and astronomy
    "space",
    "asteroid",
    "astronaut",
    "nasa",
    "astronomy",
    # Special days
    "pi day",
    "stem ",
    "women in science",
    "girls in ict",
    "world science",
    "innovation",
]

# Culture-related keywords
CULTURE_KEYWORDS = [
    # Arts
    "art",
    "arts",
    "music",
    "dance",
    "theatre",
    "theater",
    "poetry",
    "literature",
    "book",
    "reading",
    "library",
    "museum",
    # Heritage
    "heritage",
    "culture",
    "cultural",
    "tradition",
    "indigenous",
    "language",
    "mother tongue",
    # Human rights and social
    "human rights",
    "peace",
    "democracy",
    "freedom",
    "justice",
    "equality",
    "women",
    "girl",
    "child",
    "youth",
    "elderly",
    "refugee",
    "migrant",
    # Environment
    "environment",
    "earth",
    "climate",
    "ocean",
    "water",
    "forest",
    "biodiversity",
    "wildlife",
    # Education
    "education",
    "literacy",
    "teacher",
    # Food and agriculture
    "food",
    "hunger",
    "agriculture",
    "farmer",
]


def categorize_by_keywords(name: str, description: str = "") -> str:
    """
    Categorize a special day based on keywords in name and description.

    Args:
        name: Name of the special day/holiday
        description: Optional description text

    Returns:
        Category string: "Global Health", "Tech", "Culture", or "Culture" as default
    """
    text = f"{name} {description}".lower()

    # Check Health first (highest priority for health-related days)
    for keyword in HEALTH_KEYWORDS:
        if keyword in text:
            return "Global Health"

    # Check Tech
    for keyword in TECH_KEYWORDS:
        if keyword in text:
            return "Tech"

    # Check Culture
    for keyword in CULTURE_KEYWORDS:
        if keyword in text:
            return "Culture"

    # Default to Culture for general observances
    return "Culture"
